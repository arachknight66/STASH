    ...
    <>
      <main className="p-6 max-w-4xl mx-auto">
        <div className="mb-10 flex justify-between items-end flex-wrap gap-4">
          ...
          <button onClick={openCreate} className="interactive-lift bg-primary-container border-4 border-inverse-surface p-4 hard-shadow flex items-center gap-2">
        ...
        {/* Hero stat */}
        <div className="bg-tertiary-container border-4 border-inverse-surface p-6 mb-12 hard-shadow relative overflow-hidden">
        ...
        <div className="mb-6 flex flex-wrap items-center gap-3">
          {(['all', 'close', 'building', 'fresh'] as Filter[]).map((f) => (
            <button key={f} onClick={() => setFilter(f)}
              className={`border-2 border-inverse-surface px-4 py-2 font-headline font-black text-xs uppercase transition-colors ${filter === f ? 'bg-primary-container' : 'bg-white hover:bg-surface-container'}`}>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {filtered.map((b) => {
              ...
              return (
                <div key={b.id} className={`${b.isFeatured ? 'md:col-span-2' : ''} ${theme.card} border-4 border-inverse-surface p-6 hard-shadow interactive-lift flex flex-col cursor-pointer group`}>
                  ...
                  <div className="mt-auto">
                    <div className={`w-full h-12 border-4 border-inverse-surface ${theme.surface} overflow-hidden flex`}>
                      <div className={`h-full ${theme.fill} border-r-4 border-inverse-surface transition-all duration-700`} style={{ width: `${progress}%` }} />
                    </div>
                  ...
                  <div className="mt-4 pt-3 border-t border-inverse-surface/20 flex gap-2 justify-end">
                    <button onClick={(e) => { e.stopPropagation(); openBoost(b); }}
                      className="text-xs font-black uppercase border-2 border-inverse-surface px-3 py-1 bg-white hover:bg-primary-container transition-colors">
                      Boost +
                    </button>
                    <button onClick={(e) => { e.stopPropagation(); deleteBucket.mutate(b.id, { onSuccess: () => showToast(`${b.name} removed.`) }); }}
                      className="text-xs font-black uppercase border-2 border-inverse-surface px-3 py-1 bg-white hover:bg-error-container transition-colors">
                      Remove
                    </button>
                  </div>
                </div>
      ...
      }} className="interactive-lift fixed bottom-28 right-6 z-40 bg-secondary p-5 border-4 border-inverse-surface hard-shadow">
